    ...
    els.tabButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (targetId === 'tab-dashboard') {
                ...
                populateCoffreClientsDatalist();
            // --- AJOUT POUR TEMPLATES ---
            } else if (targetId === 'tab-templates') {
                renderTemplatesTable();
                populateTemplatesClientFilter();
                populateTemplatesClientsDatalist();
                populateTemplatesProjectsSelect();
            }
            // --- FIN AJOUT ---
        });
    });
...
// --- TEMPLATES : GESTION DES DOCUMENTS ---
...
// Conversion de la taille en format lisible
...
// Obtient l'icône Lucide correspondant au type de fichier
...
// Remplit la datalist des clients pour les templates
function populateTemplatesClientsDatalist() {
    const templates = getTemplatesState();
    const datalist = document.getElementById('templates-clients-datalist');
    if (!datalist) return;

    const clients = [...new Set(templates.map(t => t.client).filter(Boolean))].sort();
    datalist.innerHTML = clients.map(c => `<option value="${c}"></option>`).join('');
}

// Remplit le select des projets
function populateTemplatesProjectsSelect() {
...
// Remplit le filtre des clients
function populateTemplatesClientFilter() {
    const templates = getTemplatesState();
    const select = document.getElementById('templates-filter-client');
    if (!select) return;
...
// Rendu de la table des templates
function renderTemplatesTable() {
    ...
    const templates = getTemplatesState();
    const searchVal = (document.getElementById('templates-search')?.value || '').toLowerCase().trim();
    const typeFilter = document.getElementById('templates-filter-type')?.value || '';
    const clientFilter = document.getElementById('templates-filter-client')?.value || '';
    ...
    const filtered = templates.filter(t => {
        ...
        const matchClient = !clientFilter || t.client === clientFilter;
    ...
    tbody.innerHTML = filtered.map((doc, index) => {
        const fileIcon = getFileIcon(doc.filename || doc.name);
        const projectName = doc.projectId
            ? state.projects.find(p => p.id === doc.projectId)?.name || '—'
            : '—';
        ...
        return `
            <tr data-id="${doc.id}">
                ...
                <td>
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        <i data-lucide="${fileIcon}" style="width: 18px; height: 18px; color: #0A6E89;"></i>
                        <span style="font-weight: 600;">${doc.name}</span>
                        ${doc.description ? `<span style="font-size: 0.75rem; color: var(--text-muted);">— ${doc.description}</span>` : ''}
                    </div>
                </td>
                <td><span class="badge badge-status" style="background: #F6C900; color: #1a1a1a;">${doc.type || '—'}</span></td>
                <td>${doc.client || '—'}</td>
                <td>${projectName}</td>
                <td>${doc.size ? formatFileSize(doc.size) : '—'}</td>
                <td style="font-size: 0.8rem; color: var(--text-muted);">${doc.createdAt ? new Date(doc.createdAt).toLocaleDateString('fr-FR') : '—'}</td>
                <td style="text-align: right;">
                    <div style="display: flex; gap: 0.4rem; justify-content: flex-end;">
                        <button class="btn btn-secondary btn-icon-only templates-view-btn" data-id="${doc.id}" title="Consulter / Télécharger">
                            <i data-lucide="eye"></i>
                        </button>
                        <button class="btn btn-secondary btn-icon-only templates-replace-btn" data-id="${doc.id}" title="Remplacer le fichier">
                            <i data-lucide="refresh-cw"></i>
                        </button>
                        <button class="btn btn-secondary btn-icon-only templates-edit-btn" data-id="${doc.id}" title="Modifier les infos">
                            <i data-lucide="edit-3"></i>
                        </button>
                        <button class="btn btn-secondary btn-icon-only templates-delete-btn" data-id="${doc.id}" title="Supprimer" style="color: var(--weather-danger); border-color: rgba(239, 68, 68, 0.2);">
                            <i data-lucide="trash-2"></i>
                        </button>
                    </div>
                </td>
...
// Attache les événements aux boutons de la table des templates
function attachTemplatesEventListeners() {
    ...
    document.querySelectorAll('.templates-replace-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const id = btn.dataset.id;
    ...
    // Modifier les infos
    document.querySelectorAll('.templates-edit-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const id = btn.dataset.id;
            editTemplate(id);
        });
    ...
    // Supprimer
    document.querySelectorAll('.templates-delete-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const id = btn.dataset.id;
            deleteTemplate(id);
        });
...
// Visualise un fichier (téléchargement ou affichage dans le navigateur)
function viewTemplateFile(id) {
    const templates = getTemplatesState();
    ...
    if (!doc.fileData) {
        showToast("Le fichier n'est pas disponible.", "error");
        return;
    }
    ...
    try {
        ...
        // Pour les PDF, on les ouvre dans un nouvel onglet
        ...
        showToast(`Téléchargement de "${doc.name}" en cours...`, "success");
    } catch (err) {
...
function replaceTemplateFile(id) {
    const templates = getTemplatesState();
    const doc = templates.find(t => t.id === id);
    if (!doc) {
        showToast("Document non trouvé.", "error");
    ...
    const input = document.createElement('input');
    input.type = 'file';
    input.accept = '.doc,.docx,.pdf,.ppt,.pptx,.xls,.xlsx,.txt,.md,.odt,.ods';
    ...
    input.addEventListener('change', (e) => {
        ...
        reader.onload = (evt) => {
            ...
            saveState();
            renderTemplatesTable();
            showToast(`Fichier "${file.name}" remplacé avec succès.`, "success");
            document.body.removeChild(input);
        };
        reader.onerror = () => {
            showToast("Erreur lors de la lecture du fichier.", "error");
...
// Modifier les informations d'un document (ouvre le formulaire)
function editTemplate(id) {
    ...
    // Pré-remplir le formulaire
    document.getElementById('templates-edit-id').value = doc.id;
    document.getElementById('templates-name').value = doc.name;
    document.getElementById('templates-type').value = doc.type || '';
    document.getElementById('templates-client').value = doc.client || '';
    document.getElementById('templates-project').value = doc.projectId || '';
    document.getElementById('templates-description').value = doc.description || '';
    document.getElementById('templates-file').value = '';
    document.getElementById('templates-file-info').textContent = doc.filename ? `Fichier actuel : ${doc.filename}` : 'Aucun fichier';

    // Changer le titre et le bouton
    document.getElementById('templates-form-title').textContent = 'Modifier le document';
    document.getElementById('templates-form-submit').innerHTML = '<i data-lucide="save"></i> Mettre à jour';

    // Afficher le formulaire
    document.getElementById('templates-form-container').style.display = 'block';
    document.getElementById('templates-form-container').scrollIntoView({ behavior: 'smooth' });
...
function deleteTemplate(id) {
    ...
    const confirmed = confirm(`Supprimer définitivement le document "${doc.name}" ?`);
    if (!confirmed) return;
    ...
    populateTemplatesClientFilter();
    populateTemplatesClientsDatalist();
    showToast(`Document "${doc.name}" supprimé.`, "info");
}

// Réinitialise le formulaire d'ajout de template
function resetTemplatesForm() {
    document.getElementById('templates-form').reset();
    document.getElementById('templates-edit-id').value = '';
    document.getElementById('templates-file-info').textContent = 'Aucun fichier sélectionné';
    document.getElementById('templates-form-title').textContent = 'Ajouter un document';
    document.getElementById('templates-form-submit').innerHTML = '<i data-lucide="save"></i> Enregistrer';
    document.getElementById('templates-form-container').style.display = 'none';
}

// Setup des événements pour les templates
function setupTemplatesListeners() {
    // Bouton Ajouter
    document.getElementById('templates-add-btn').addEventListener('click', () => {
        resetTemplatesForm();
        document.getElementById('templates-form-container').style.display = 'block';
        document.getElementById('templates-form-container').scrollIntoView({ behavior: 'smooth' });
        populateTemplatesClientsDatalist();
        populateTemplatesProjectsSelect();
        document.getElementById('templates-name').focus();
    });

    // Annuler le formulaire
    document.getElementById('templates-form-cancel').addEventListener('click', resetTemplatesForm);

    // Effacer la sélection de fichier
    document.getElementById('templates-file-clear').addEventListener('click', () => {
        document.getElementById('templates-file').value = '';
        document.getElementById('templates-file-info').textContent = 'Aucun fichier sélectionné';
    });

    // Afficher le nom du fichier sélectionné
    document.getElementById('templates-file').addEventListener('change', (e) => {
        const file = e.target.files[0];
        if (file) {
            document.getElementById('templates-file-info').textContent = `${file.name} (${formatFileSize(file.size)})`;
        } else {
            document.getElementById('templates-file-info').textContent = 'Aucun fichier sélectionné';
        }
    });
    ...
    document.getElementById('templates-form').addEventListener('submit', (e) => {
        ...
        const editId = document.getElementById('templates-edit-id').value;
        const name = document.getElementById('templates-name').value.trim();
        const type = document.getElementById('templates-type').value;
        const client = document.getElementById('templates-client').value.trim();
        const projectId = document.getElementById('templates-project').value;
        const description = document.getElementById('templates-description').value.trim();
        const fileInput = document.getElementById('templates-file');

        if (!name) {
            showToast("Veuillez saisir un nom pour le document.", "error");
            return;
        }
        if (!type) {
            showToast("Veuillez sélectionner un type de document.", "error");
            return;
        }
        ...
        if (editId) {
            ...
            // Remplacer le fichier si un nouveau est sélectionné
            if (fileInput.files.length > 0) {
                ...
                reader.onload = (evt) => {
                    ...
                    populateTemplatesClientsDatalist();
                    resetTemplatesForm();
                    showToast(`Document "${name}" mis à jour.`, "success");
                };
                reader.readAsDataURL(file);
                return; // Le reste sera fait dans le callback
            }
        ...
        reader.onload = (evt) => {
            ...
            state.templates.push(newDoc);
            saveState();
            renderTemplatesTable();
            populateTemplatesClientFilter();
            populateTemplatesClientsDatalist();
            resetTemplatesForm();
            showToast(`Document "${name}" ajouté avec succès.`, "success");
        };
        reader.readAsDataURL(file);
    });

    // Filtres et recherche
    document.getElementById('templates-search').addEventListener('input', renderTemplatesTable);
    document.getElementById('templates-filter-type').addEventListener('change', renderTemplatesTable);
    document.getElementById('templates-filter-client').addEventListener('change', renderTemplatesTable);

    // Réinitialiser les filtres
    document.getElementById('templates-clear-filters').addEventListener('click', () => {
        document.getElementById('templates-search').value = '';
        document.getElementById('templates-filter-type').value = '';
        document.getElementById('templates-filter-client').value = '';
        renderTemplatesTable();
        showToast("Filtres réinitialisés.", "info");
    });

    // Export de la liste des documents (sans les données binaires)
    document.getElementById('templates-export-btn').addEventListener('click', () => {
        const templates = getTemplatesState();
        ...
        // Exporter sans les données binaires pour alléger
        const exportData = templates.map(({ id, name, type, client, projectId, description, filename, size, mimeType, createdAt, updatedAt }) => ({
            id, name, type, client, projectId, description, filename, size, mimeType, createdAt, updatedAt
        }));
        ...
        showToast(`${templates.length} document(s) exporté(s).`, "success");
    });

    // Import de documents (JSON)
    document.getElementById('templates-import-btn').addEventListener('click', () => {
        ...
        reader.onload = (evt) => {
            try {
                ...
                if (!Array.isArray(imported) || imported.length === 0) {
                    showToast("Format invalide ou liste vide.", "error");
                    return;
                }

                const confirmMsg = `Importer ${imported.length} document(s) ?\n\nLes documents existants seront conservés.`;
                if (!confirm(confirmMsg)) return;

                const templates = getTemplatesState();
                // Générer de nouveaux IDs pour éviter les conflits
                imported.forEach(doc => {
                    doc.id = 'template-' + Date.now() + '-' + Math.random().toString(36).substr(2, 4);
                    doc.createdAt = doc.createdAt || new Date().toISOString();
                    doc.updatedAt = doc.updatedAt || new Date().toISOString();
                    // Les données binaires ne sont pas incluses dans l'export, donc on laisse vide
                    doc.fileData = '';
                    templates.push(doc);
                });
                ...
                populateTemplatesClientFilter();
                populateTemplatesClientsDatalist();
                showToast(`${imported.length} document(s) importé(s).`, "success");
            } catch (err) {
                showToast("Erreur lors de la lecture du fichier.", "error");
                console.error(err);
            }
