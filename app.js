...
function encryptPassword(password) {
    ...
    // encodeURIComponent/unescape convertit d'abord la chaîne en octets UTF-8,
    // ce qui évite l'erreur "InvalidCharacterError" de btoa() dès que le mot
    // de passe contient un caractère accentué ou hors de la plage Latin1.
...
function setupCoffreListeners() {
    ...
    document.getElementById('coffre-toggle-password').addEventListener('click', function () {
        const input = document.getElementById('coffre-motdepasse');
        const btn = this;
        // On régénère systématiquement un <i data-lucide="..."> frais plutôt que de
        // chercher l'élément <i> existant : Lucide le remplace par un <svg> après le
        // premier rendu (refreshIcons), donc un querySelector('i') renvoie null ensuite.
